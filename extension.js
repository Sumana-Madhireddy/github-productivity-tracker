...
async function getOrCreateRepo(token) {
  ...
  if (!username) {
    vscode.window.showErrorMessage("Failed to get GitHub username in get or create!");
    return;
  }
	...
	try {
	  ...
	  if (error.response) {
		if (error.response.status === 422) {
		  ...
		  vscode.window.showErrorMessage(`GitHub API error: ${error.response.status} - ${error.response.data.message}`);
		}
	  } else {
		console.error("Error during repository creation:", error);
		vscode.window.showErrorMessage("Failed to create repository. Please check your network connection and try again.");
	  }
	  throw error;
	}
}
...
async function commitLogToRepo(token, logContent, taskNumber, recentCommitSummaries) {
	...
	const username = await getUserUsername(token);
	if (!username) {
	  vscode.window.showErrorMessage("Failed to get GitHub username in commitLogto Repo");
	  return;
	}
